...
namespace AttandanceSyncApp.Controllers
{
    ...
    public class AdminUserToolsController : BaseController
    {
        ...
        public JsonResult AssignTool(int userId, int toolId)
        {
            try
            {
                ...
                if (tool == null)
                    return Json(ApiResponse.Fail("Tool not found"));

                var existing = _unitOfWork.UserTools
                    .GetAll()
                ...
                if (existing != null)
                    return Json(ApiResponse.Fail("Tool already assigned"));

                // 🔥 FIX HERE
                var adminUser = _unitOfWork.Users
                    .GetAll()
                    .FirstOrDefault(x => x.Id == CurrentUserId);

                if (adminUser == null)
                    return Json(ApiResponse.Fail("Admin user not found"));

                var userTool = new UserTool
                {
                    UserId = userId,
                    ToolId = toolId,
                    AssignedBy = CurrentUserId > 0 ? CurrentUserId : (int?)null,   // safe FK
                ...
                return Json(ApiResponse.Success("Tool assigned successfully"));
            }
            catch (Exception ex)
            {
                ...
                return Json(ApiResponse.Fail(fullError));
            }
        }
